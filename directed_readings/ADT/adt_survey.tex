...
The overarching data model behind Python is that everything is either an object or a relation between objects \cite{pythonDataModel}.
...
of Pydantic \cite{pydantic} will enable both type hints and data validation for those types \textit{at runtime}. The Pydantic version
...
and IDEs are able to assist in new code development through intellisense and autocompletion of methods and variables \cite{typescriptHandbook}.
...
\begin{itemize}
    ...
    a union of sets, or rarely even an intersection of sets \cite{typescriptForOOP}.
    \item Types are structural \cite{typescriptForFunc}. Any object containing the required fields needed to be a member of a certain type
    ...
    with the only allowed value being the literal itself \cite{typescriptHandbook}.
...
For a non-inductive approach to the \texttt{LinkedList} data type, we can use TypeScript's ``interface'' structure to create something that
looks much more like the Pydantic version:
\inputminted{typescript}{linked_list/main_interface.ts}

Finally, a pure OOP version of our \texttt{LinkedList} type, using classes, can be formed.
...
Rust is a unique, high-performance language focused on efficiency and reliability \cite{rustBook}.
...
must be carefully considered. The guarantees of Rust in relation to memory enable ``fearless concurrency'' \cite{rustBook}, or in other words,
...
Haskell is a pure, lazy functional language with excellent support for concise function composition, elegant recursion, and powerful typing \cite{learnHaskell}.
...
guarantee useful properties of a program. As we explore in Section 2.6, attempting these kinds of proofs
...
these interfaces can broadly fall into two categories \cite{learnHaskell}:
\begin{enumerate}
...
are its guarantees regarding the termination of well-formed programs and the types of function outputs \cite{agda}.
...
In Dafny, the act of verifying a program is as natural and important as writing the program itself \cite{dafny}.
...
the co-inductive data type \cite{dafnyRef}. Since these types may be theoretically infinite,
...
and proving the correctness of software following the model specification \cite{eventB}.

Data types for Event-B may not exist in the normal sense of the word, but a different approach to data abstraction
through the concepts of machines, contexts, and events is more than suitable for the same purpose \cite{eventBBook}:
\begin{itemize}
...
encourages the use of exceptions and try-catch structures to create and contain errors \cite{python}.
...
to panic and act like an exception was thrown \cite{rustBook}. However, the key difference between panics and exceptions
...
functions is provably correct \cite{agda}. The prevention of errors before they can even exist can be a powerful asset
...
According to an article released by the Rust language team in 2017 \cite{rust2017}, the ergonomic challenges
of language features can be summarized by striking a balance between explicitness and implicitness.
...
the foundation of natural language allows variables and functions to have meaning, even outside of the code itself.
For example, Python's \texttt{len()} function is likely to resonate with more people than APL's symbolic-based $\rho$ operator.
