...
% Optimization stage ideas
% - use another modified grammar to define rewrite rules (a language that looks similar to the source language but much smaller, basically rewrite strategies). The language could have variables to represent abstraction over internal structure. For example:
% a: Var[int]
% a + a - a = a
% a + 0 = a
% - embed rewrites directly through AST translations, ie. Sub(Add(1, 1),1) => 1
% these two methods kind of combine egglog's strengths and weaknesses, cant use egglog directly because of lack of lib maturity and inflexible type support

% mention eqsat vs rewriting strategies, etc

% Then comes choosing the representation of data. Assume we have a mostly optimized setup, but some optimizations depend heavily on the internal representation (eg. bi-directional maps, "sorted" sets).

% \subsubsection{Equality Saturation with Egg(log)}
% \subsubsection{Rewrite Rules for Common Expression Structures}
% TODO move this either to the bullet point list or to a few paragraphs ahead
...
% Present a list of set based rules in the first format
\section{Conclusion}
