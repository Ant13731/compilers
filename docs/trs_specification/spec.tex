...
\begin{align}
  \tag{Union}
  S \cup T &\rightarrow \bag{(x, a+b)}{(x,a) \in S \land (x,b) \in T}{a, b \geq 0}
  \\
  \tag{Intersection}
...
Different implementations of each data type will have varying strengths and weaknesses. With regards to implementation of the data structure representing these types, we will need to consider the theoretical asymptotic times in addition to smaller, concrete tests. Since this document is only concerned with the theoretical compiler specification, we examine only the theoretical time and space complexity. However, paired with the gathered examples, we outline a plan to test the real-world usage of these operations.

A first approach to tackling these type representations would likely constitute a linked list. The space requirements for enumeration are straightforward, with extra allocations for pointers to subsequent elements. Insertions for unordered collections are $O(1)$, and $O(n)$ for sequences. Lookups for all collections are $O(n)$, an unfortunate trait that does not pair well with the often-used \texttt{in} operator for set-generated code.

On the other hand, hashmaps, with $O(1)$ membership lookups and element lookups, are useful for all unordered collections. Relations may need bidirectional hashmaps that can efficiently handle many-to-many relations.
