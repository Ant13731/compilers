...
\begin{align}
  ...
  % \\
  \tag{Generator Selection \footnote{The LH term must occur inside a quantifier's predicate - one generator per or-clause. $P_g$ is the generator, a single clause distinguished from the rest of $\bigwedge P_i$. $P_g$ must be a set membership operation, of the form $x \in S$.}}
  \bigwedge P_i
  &\leadsto
  P_g \land \bigwedge_{P_i \neq P_g} P_i
  \\
  \tag{Equality Separation \footnote{All $P_eq$ equalities must follow that one argument contains a single, free identifier not defined the the current scope/context of this quantifier. These equalities will be converted into substitutions.}}
  P_g \land \bigwedge P_i
  &\leadsto
  P_g \land \bigwedge P_{eq} \land \bigwedge_{P_i \neq P_g \land P_i \notin \bigcup P_{eq}} P_i
  \\
  \tag{Equality Substitution \footnote{The LHS of $P_{eq}$ must match the form of $Identifier(x) = e$}}
  P_g \land \bigwedge P_{eq} \land \bigwedge P_i
  &\leadsto
  P_g \land \bigwedge P_i[P_{eq}.identifier := P_{eq}.e]
  \\
...
\begin{align}
  ...
  \oplus E \mid P % TODO add side condition that P contains the dummy variables/ generators. Give pre and post conditions for each phase based on the internal structure of variables (ex. P at this point should be that form of the AND-triple)
  &\leadsto
