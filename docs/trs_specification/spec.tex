...
\begin{itemize}
  \item All terms with a set-like type (relations, bags, sets, sequences, etc.) must be in comprehension form.
  \item Each top-level or-clause within quantification predicates must have one selected `generator' predicate (GeneratorSelectedPredicate - GSP) of the form $x \in S$ that loops over its bound dummy variable $x$.
...
\begin{align}
  ...
  GSP(P_g, \bigwedge P_i)
  &\leadsto
  GSP(P_g, \bigwedge_{free(P_i)} P_i \land GSP(P_{g_2}, \bigwedge_{\lnot free(P_i) \land P_i \neq P_{g_2}} P_i))
\end{align}
\end{minipage}

\subsubsection{Reduce Duplicate Generators}
...
\begin{itemize}
  \item All terms with a set-like type (relations, bags, sets, sequences, etc.) must be in comprehension form.
  \item Quantifier predicates are in disjunctive partial-normal form (with GSP replacing AND structures). Generator predicates can "factor" one or more or-clauses. The expected predicate shape is $\bigvee GSP(P_g, P)$
  % The expected predicate shape is $\bigvee GSP(P_g, \bigvee \bigwedge P_i)$, where $P_i$ is either a boolean statement or a $GSP(P_g, \bigvee ...)$ structure itself.
...
\noindent\begin{minipage}{\linewidth}
\begin{align}
  \tag{Reduce Duplicate Generators \footnote{The LH term must occur inside a quantifier's predicate. Combines clauses with the same generator. Requires Generator Selection to be run first.
  % \\We leave this optimization out for now, given difficulties with counterexample $\bSetT{s}{(x \in \{1, 2\} \land (s = x + 1 \lor s = x + 2)) \lor (x \in \{4\} \land s = x + 1)}$.
  }}
  GSP(P_g, P) \lor GSP(P_g, Q)
  &\leadsto
  GSP(P_g, P \lor Q)
  % \\
  % \tag{GSP And-wrapping}
  % GSP(P_g, P) % where P is of the form GSP(...), not wrapped with And or Or?
  % &\leadsto
  % GSP(P_g, \bigwedge P)
  % \\
  % \tag{GSP Or-wrapping}
